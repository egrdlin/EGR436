...
{
    // Setting to Sunday, November 11, 2011 at 11:11:11 for testing
...
void RTC_read(char *date) //EGR 326 lecture slide
{
    //uint8_t tUBYTE = 0;
    //uint8_t tLBYTE = 0;
    ...
    while (MAP_I2C_isBusBusy(EUSCI_B1_BASE));
    // read from RTC registers (pointer auto increments after each read)
    date[0]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[1]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[2]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[3]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[4]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[5]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
    date[6]=MAP_I2C_masterReceiveSingleByte(EUSCI_B1_BASE);
